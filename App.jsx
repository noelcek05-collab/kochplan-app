...
export default function App() {
  ...
  const Btn = ({ onClick, disabled, children, primary, small, style = {} }) => (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      border: primary ? 'none' : `1px solid ${C.bd}`, background: primary ? C.acc : C.surf,
  ...
  const Seg = ({ value, set, opts }) => (
    <div style={{ display: 'flex', gap: 4, background: C.surf2, padding: 4, borderRadius: 100, flexWrap: 'wrap' }}>
      {opts.map(([v, l]) => (
        <button key={v} onClick={() => set(v)} style={{
          flex: '1 1 auto', padding: '7px 12px', border: 'none', borderRadius: 100, cursor: 'pointer',
          background: value === v ? C.surf : 'transparent', color: value === v ? C.ink : C.ink3,
          fontSize: 12.5, fontFamily: 'var(--font-body)', fontWeight: 500, transition: 'all .15s',
          boxShadow: value === v ? (dark ? '0 1px 4px rgba(0,0,0,.3)' : '0 1px 4px rgba(0,0,0,.06)') : 'none', whiteSpace: 'nowrap'
        }}>{l}</button>
      ))}
    </div>
  );
  const Stepper = ({ v, set, min, max }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <button onClick={() => set(Math.max(min, v - 1))} style={{ width: 30, height: 30, borderRadius: '50%', border: 'none', background: C.surf2, color: C.ink, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Minus size={14} /></button>
      <span style={{ fontFamily: 'var(--font-display)', fontSize: 19, fontWeight: 500, minWidth: 22, textAlign: 'center', color: C.ink }}>{v}</span>
      <button onClick={() => set(Math.min(max, v + 1))} style={{ width: 30, height: 30, borderRadius: '50%', border: 'none', background: C.surf2, color: C.ink, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Plus size={14} /></button>
    </div>
  );
  const Field = ({ label, children }) => (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontSize: 12, color: C.ink3, fontWeight: 600, letterSpacing: '.04em', textTransform: 'uppercase', marginBottom: 9 }}>{label}</div>
      {children}
    </div>
  );
  const Card = ({ children, style = {}, className }) => (
    <div className={className} style={{ background: C.surf, border: `1px solid ${C.bd}`, borderRadius: 16, padding: '20px 22px', ...style }}>{children}</div>
  );
  const Tab = ({ id, label, Icon }) => (
    <button onClick={() => { setTab(id); setActive(null); }} style={{
      flex: 1, padding: '9px 4px', border: 'none', background: 'transparent', cursor: 'pointer',
      color: tab === id ? C.ink : C.ink3, fontSize: 11, fontFamily: 'var(--font-body)', fontWeight: 500,
      ...
      <span>{label}</span>
      {tab === id && <div style={{ position: 'absolute', bottom: -1, left: '22%', right: '22%', height: 2, background: C.acc, borderRadius: 2 }} />}
    </button>
  );
  const ChatBox = ({ msgs, loading, input, setInput, send, placeholder, hint, endRef, minH = 200 }) => (
    <div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9, minHeight: minH, marginBottom: 12 }}>
        {msgs.length === 0 && <div style={{ textAlign: 'center', padding: '28px 16px', color: C.ink3, fontSize: 13, fontStyle: 'italic', fontFamily: 'var(--font-display)', lineHeight: 1.7, whiteSpace: 'pre-line' }}>{hint}</div>}
        {msgs.map((m, i) => (
          <div key={i} style={{
            alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '86%',
            background: m.role === 'user' ? C.accSoft : C.surf, color: m.role === 'user' ? C.accInk : C.ink,
        ...
        {loading && <div style={{ alignSelf: 'flex-start', background: C.surf, border: `1px solid ${C.bd}`, borderRadius: '4px 14px 14px 14px', padding: '11px 15px', fontSize: 13, color: C.ink3, fontStyle: 'italic', fontFamily: 'var(--font-display)' }}>schreibt…</div>}
        <div ref={endRef} />
      </div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
        <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder={placeholder} rows={2} style={{ flex: 1, resize: 'none', fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.5, padding: '11px 14px', borderRadius: 10, border: `1px solid ${C.bd}`, background: C.surf, color: C.ink, outline: 'none' }} />
        <Btn onClick={send} disabled={!input.trim() || loading} primary style={{ padding: '11px 15px' }}><Send size={14} /></Btn>
      </div>
    </div>
  );
